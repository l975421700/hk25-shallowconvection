...
axes.coastlines()
axes.set_title(f'UK N2560 RAL3 healpix')
plt.savefig('figures/healpix-test.png')
...
this_nside = hp.get_nside(um.rsut.mean(dim='time'))
...
plt_colnames = ['CERES', 'ERA5', 'ERA5-CERES', 'UM-CERES']
...
axs[1].pcolormesh(
        era5_out.lon, era5_out.lat,
        era5_out.mean(dim='time'),
        norm=pltnorm, cmap=pltcmp, transform=ccrs.PlateCarree())
